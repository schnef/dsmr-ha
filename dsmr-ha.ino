...
void setup() {
  ...
  // A telegram always should start with a '/'
  buffer[0] = '/';
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (c == '/') {
      digitalWrite(LED_BLUE, ON);
      n = Serial.readBytesUntil('!', &buffer[1], TLGSIZE);
//      yield(); // TODO: is this needed?
      if (n < TLGSIZE) {
        // The '!' was consumed, so insert it instead
        buffer[n+1] = '!';
        m = Serial.readBytes(crc, CRCSIZE);
        crc1 = strtol(crc, NULL, 16);
        crc2 = crc16(&buffer[1], n+1);
        if (crc1 == crc2) {
          if (client.connected()) {
            digitalWrite(LED_RED, ON);
            // Add crc and terminating CR/LF and string end
            buffer[n+2] = crc[0];
            buffer[n+3] = crc[1];
            buffer[n+4] = crc[2];
            buffer[n+5] = crc[3];
